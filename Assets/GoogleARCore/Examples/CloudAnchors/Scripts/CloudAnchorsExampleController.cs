...
#pragma warning restore 618
        ...
        public SpawningHandler spawningHandler;
...
#pragma warning restore 618
        ...
        public void Update()
        {
            ...
            if (Application.platform != RuntimePlatform.IPhonePlayer)
            {
                        ...
                        TrackableHitFlags.PlaneWithinPolygon, out arcoreHitResult))
                {
                    ...
                    Debug.Log(JsonUtility.ToJson(m_LastHitPose, true));
                    //if (isPlaneDetected == false)
                    //{
                    detectedPlane = arcoreHitResult.Trackable as DetectedPlane;
                    m_LastHitPose = new Pose(detectedPlane.CenterPose.position, Quaternion.identity);
                    Debug.Log("===========================================");
                    Debug.Log(JsonUtility.ToJson(detectedPlane.CenterPose.position, true));
                    //}
                }
            }
            else
            ...
            if (m_LastHitPose != null)
            {
                ...
                if (_CanPlaceStars())
                {
                    spawningHandler.lol_Update();
                    //scoreboard.SetSelectedPlane(detectedPlane);

                    //scoreboard.CreateTable(worldPose);
                    _InstantiateStar();
                }
                else if (!IsOriginPlaced && m_CurrentMode == ApplicationMode.Hosting)
                {
                    ...
                    {
                        m_WorldOriginAnchor =
                            arcoreHitResult.Trackable.CreateAnchor(new Pose((detectedPlane.CenterPose.position), Quaternion.identity));
                        m_WorldOriginAnchor.transform.rotation = Pose.identity.rotation;
                        m_WorldOriginAnchor.transform.position = detectedPlane.CenterPose.position;
                    }
                    else
                    {
                    ...
                    //worldPose = SetWorldOrigin(m_WorldOriginAnchor.transform);
        ...
        IEnumerator waiter()
        {
            ...
            scoreboard.CreateTable();
            NetworkServer.Spawn(scoreboard.gameplay);
            NetworkServer.Spawn(scoreboard.world);
            loader.lol_Awake();
        }
        ...
        public void SetWorldOrigin(Transform anchorTransform)
        {
            ...
            if (Application.platform != RuntimePlatform.IPhonePlayer)
            {
                ...
                //detectedPlane.transform.SetPositionAndRotation(worldPose.position, worldPose.rotation);
            }
            else
            {
